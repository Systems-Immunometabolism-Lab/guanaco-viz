...
The application provides two complementary layers of cell selection that work together to enable precise data analysis:

Layer 1: Global Data Filter (Categorical Metadata Selection)

The Global Data Filter controls which cells are displayed in the
embedding plots and serves as the foundation for all subsequent
selections.

How to Use:

1. Access the Filter Panel: Click "▼ Show Filters" below the cell count
display
2. Select Categories: Use dropdown menus for each metadata column (e.g.,
cell type, treatment, batch)
   - Multiple values can be selected per category
   - All categories are selected by default
3. Quick Actions:
   - "Select All": Restore all options for all categories
   - "Clear All": Deselect all options (useful for starting fresh)
4. Apply Changes: Click "Apply Filter" to update the embedding plots

Layer 2: Lasso Selection on Embedding

Once the Global Data Filter determines which cells are visible, you can
make precise selections directly on the embedding plots.

How to Use:

1. Select Tool: Use plotly's lasso selection tool (icon in plot toolbar)
2. Draw Selection: Click and drag to draw a lasso around cells of
interest
3. View Selection: Selected cells appear highlighted while others become
dimmed
4. Update Other Plots: Click the "Update Other Plots" button to apply
selection to other visualizations

Layered Selection Logic

The selection system follows this hierarchy:

All Cells (100%)
   ↓ Global Data Filter (Layer 1)
Filtered Cells (shown in embedding)
   ↓ Lasso Selection (Layer 2)
Selected Cells (passed to other plots)

Important Notes:

1. Independence: Global Data Filter affects only embedding plots, while
lasso selection affects other plots (heatmaps, violin plots, etc.)
2. Selection Persistence:
   - Lasso selections are temporary and reset when you change
genes/metadata
   - Global filters persist until manually changed
3. Default Behavior:
   - If no lasso selection is made, clicking "Update Other Plots" passes
all filtered cells to other plots
   - This allows quick analysis of globally filtered data
4. Cell Index Mapping: The system correctly maps cell indices between
different plot types:
   - Continuous data (genes): Uses customdata with original cell indices
   - Categorical data: Maps through category membership and point numbers

Workflow Example:

1. Initial Filtering: Use Global Data Filter to show only "T cells" from
"treated" samples
2. Embedding Display: UMAP now shows only relevant cells
3. Precise Selection: Use lasso to select a cluster of activated T cells
4. Detailed Analysis: Click "Update Other Plots" to see gene expression
profiles of just those activated T cells in violin plots, heatmaps, etc.
...
The DOWNLOAD dropdown menu provides two export options for your selected
cells (in this case, 560 cells selected via lasso selection):

1. Cell IDs (txt)

- Format: Plain text file (.txt)
- Content: List of cell identifiers/barcodes for the selected cells
- Use Case:
   - Import selected cell IDs into other analysis tools
   - Create custom cell subsets for downstream analysis
   - Share specific cell selections with collaborators
   - Quality control and validation of selections

2. Subset AnnData (.h5ad)

- Format: HDF5-based AnnData file (.h5ad)
- Content: Complete single-cell dataset containing only the selected
cells
- Includes:
   - Gene expression matrix for selected cells
   - All metadata (cell type, treatment, etc.) for selected cells
   - Dimensionality reduction coordinates (UMAP, PCA, etc.)
   - Any computed metrics and annotations
- Use Case:
   - Detailed analysis of cell subset in Python/scanpy
   - Share complete dataset subset with collaborators
   - Further clustering or trajectory analysis of selected population
   - Integration with other single-cell analysis pipelines
