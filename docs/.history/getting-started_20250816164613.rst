...
Select cells
^^^^^^^^^^^^^^^^^^^^^^^^^^
The application provides two complementary layers of cell selection that
work together to enable precise data analysis:
...
- Real-time Preview: Shows how many cells will be selected before
applying
- Active Cell Count: Displays current number of visible cells vs. total
- Affects Embedding Only: This filter controls what appears in UMAP/t-SNE
plots
- Persistent: Settings remain active until manually changed
...
- Visual Feedback: Selected cells remain fully visible (opacity=1),
unselected cells are dimmed (opacity=0.2)
- Flexible Selection: Works on any embedding plot (UMAP, t-SNE, PCA,
etc.)
- Works with Any Coloring: Whether colored by genes, metadata, or
co-expression
- Cell Index Tracking: Uses customdata in plots for accurate cell
identification
...
This layered approach enables both broad data exploration (via metadata
filtering) and precise analysis (via spatial selection), making it easy
to focus on specific cell populations while maintaining full analytical
power.
